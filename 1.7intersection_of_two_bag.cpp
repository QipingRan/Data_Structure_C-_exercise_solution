// The intersection of two bags is a new bag containing the entries that occur 
// in both of the original two bags. 
// Design and specify a method intersection for the ADT bag that returns 
// as a new bag the intersection of the bag receiving the call to the method and the bag 
// that is the method’s one argument. Include sufficient comments to fully specify the method.

// Note that the intersection of two bags might contain duplicate items. For example, 
// if object x occurs five times in one bag and twice in another, 
// the intersection of these bags contains x two times. Specifically, 
// suppose that bag1 and bag2 are bags; bag1 contains the strings a, b, and c;
//  and bag2 contains the strings b, b, d, and e. 
//  The expression bag1.intersection(bag2) returns a bag containing only the string b. 
...
// The following code illustrates the functionality of the “intersection()” function.
...
template MyBag MyBag::intersection(MyBag myBag2)
{
    ...
    {
    /*Check whether the items at the index is present in the bag2.*/
...
int main()
{
    ...
    //Call the method intersection().
    MyBag myBag3=myBag1.intersection(myBag2);
    //Display message.
    cout<<"";
