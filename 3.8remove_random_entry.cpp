...
// The below program illustrates the functionality of the method “removeRandom()”,
...
class BagInterface
{
    ...
    public:
        ...
        /*Method outputs true if an random entry is deleted from the bag.*/
...
ItemType ArrayBag::removeRandom()
{
    //Get the random index rIdx using rand().
    int rIdx = abs(rand() % itemCount);
    //Get the item at rIdx.
    ItemType deletedItem = items[rIdx];
    //Decrement the itemCount by 1.
    itemCount--;
    //Replace the item at rIdx with item at itemCount.
    items[rIdx] = items[itemCount];
...
int main()
{
    ...
    cout<<endl;
    //Display message.
    cout<<"Randomly removed item:"<<endl;
